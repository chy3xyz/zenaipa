...
[![Tests](https://img.shields.io/badge/tests-35%20backend%20%2B%205%20frontend-green)]()
...
| Layer | Technology |
| --- | --- |
| Backend | [Zig](https://ziglang.org) 0.17 · [zigmodu](https://github.com/chy3xyz/zigmodu) v0.20.1+ (HTTP, security, AI, resilience, Application lifecycle) · [zent](https://github.com/chy3xyz/zent) v0.67.0+ (ORM, schema, migrations) |
...
| ✅ Done | **Deps at latest, pinned by git hash** — zigmodu v0.20.1 + zent v0.67.0 locked via `git+https` URL + content hash (no sibling checkout needed); `bindJsonLoose` adopted across all 16 write endpoints; schema merged to a single `buildGraph` per zent UPGRADING §7a |
